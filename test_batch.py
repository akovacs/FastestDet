...
if __name__ == '__main__':
    ...
    img_paths_list = glob.glob(opt.img)
    assert len(img_paths_list) > 0, "请指定正确的测试图像路径"
    ...
    img_file_names = []
    imgs = []
    for img_index, img_path in enumerate(img_paths_list):
    ...
    # 模型推理
    for img_file_name, ori_img, img in zip(img_file_names, ori_imgs, imgs):
        print(img_file_name)
        ...
        cv2.imwrite(f'output-imgs/result-{img_file_name}', ori_img)
